...
public class MenuChildAdapter extends BaseAdapter {
	private List<IndexData> menuList;
	private Context context;
	private boolean IsEdit = false;

	public MenuChildAdapter(Context context, List<IndexData> list, boolean isEdit2) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final IndexData menuEntity = menuList.get(position);
		...
		viewHodler.deleteImg.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!menuEntity.isSelect()) {
					MeunManageActivity.AddMenu(menuEntity);
				}
			}
		});
