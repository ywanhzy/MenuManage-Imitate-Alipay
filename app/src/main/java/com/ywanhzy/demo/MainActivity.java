...
import com.ywanhzy.drag.LineGridView;
...
public class MainActivity extends AppCompatActivity {
    ...
    private List<IndexData> indexDataAll = new ArrayList<IndexData>();
    private List<IndexData> indexDataList = new ArrayList<IndexData>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (JsonElement indexArr : jsonArray) {
            //使用GSON，直接转成Bean对象
            IndexData indexData = gson.fromJson(indexArr, IndexData.class);
            indexDataAll.add(indexData);
        }
        ...
        List<IndexData> indexDataUser = (List<IndexData>) appContext.readObject(AppConfig.KEY_USER);
        if(indexDataUser==null||indexDataUser.size()==0) {
            appContext.saveObject((Serializable) indexDataAll, AppConfig.KEY_USER);
        }
        indexDataList = (List<IndexData>) appContext.readObject(AppConfig.KEY_USER);

        IndexData allIndexData=new IndexData();
        allIndexData.setIco("");
        allIndexData.setId("all");
        allIndexData.setTitle("全部");
        indexDataList.add(allIndexData);
        ...
        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (strId.equals("all")) {// 更多
                    intent.setClass(MainActivity.this, MeunManageActivity.class);
    ...
    protected void onResume() {
        ...
        indexDataList = (List<IndexData>) appContext.readObject(AppConfig.KEY_USER);
        IndexData allIndexData=new IndexData();
        allIndexData.setIco("all_big_ico");
        allIndexData.setId("all");
        allIndexData.setTitle("全部");
        indexDataList.add(allIndexData);
        adapter = new IndexDataAdapter(MainActivity.this, indexDataList);
